<template>
    <div>
      ...
      >
        <template v-slot:[`item.action`]="{ item }">
          ...
          <v-icon @click="conFirm(item.id)">mdi-delete</v-icon>
        </template>
      </v-data-table>
  ...
  <script>
  import deleteMember from '~/gql/mutations/deleteMember.gql'
  import updateMember from '~/gql/mutations/updateMember.gql'
    ...
    methods: {
      ...
      deleteMember(id){
        this.$apollo
          ...
          .catch((err) => {
            console.log('🚀 ~ file: index.vue:227 ~ deleteData ~ err:', err)
          })
      }
    },
    mounted() {
